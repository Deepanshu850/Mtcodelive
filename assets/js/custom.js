(function ($) {
	...
	$("#emailAdresss").on("click", function () {
		...
		$("#emailAdresss a").replaceWith(
			`<a href="${emails}"><span><i class='bx bxl-whatsapp'></i>${whatsapp}</span></a>`
		);
	});
	$("#emailAdresss2").on("click", function () {
		let emails = $(this).data("emails");
		let whatsapp = $(this).data("whatsapp");
		$("#emailAdresss2 a").replaceWith(
			`<a href="${emails}"><span><i class='bx bxl-whatsapp'></i>${whatsapp}</span></a>`
		);
	});
	$("#cardnumber").on("click", function () {
		let card = $(this).data("card");
		$("#cardnumber .card-number-show").replaceWith(
			`<div class="card-number-show"><span class="title">${card}</span></div>`
		);
	});
	$("#cardnumber2").on("click", function () {
		let card = $(this).data("card");
		$("#cardnumber2 .card-number-show").replaceWith(
			`<div class="card-number-show"><span class="title">${card}</span></div>`
		);
	});
	...
	var swiper = new Swiper(".home2-banner-slider", {
		...
		fadeEffect: { crossFade: true },
		autoplay: { delay: 2500, disableOnInteraction: false },
		pagination: { el: ".banner2-swiper-pagination", clickable: true },
	});
	...
	var swiper = new Swiper(".dream-category-slider", {
		...
		autoplay: { delay: 2500, disableOnInteraction: false },
		navigation: { nextEl: ".next-13", prevEl: ".prev-13" },
		pagination: { el: ".dream-pagination", clickable: true },
		breakpoints: {
			280: { slidesPerView: 1 },
			386: { slidesPerView: 1 },
			576: { slidesPerView: 2, spaceBetween: 15 },
	...
	$(function () {
		$(".payment-options li").on("click", function () {
			$(".payment-options li").removeClass("active");
